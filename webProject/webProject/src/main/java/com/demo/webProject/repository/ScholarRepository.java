package com.demo.webProject.repository;
...
public class ScholarRepository {
	...
	public List<ScholarModel> getAllScholars() { //Returning details of all Scholars by using List
		
	List<ScholarModel> sc=new ArrayList<>();
	try{  
		
	//Class.forName("com.mysql.jdbc.Driver");  
		Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/inductiondb?serverTimezone=UTC","root","");  
	
	Statement stmt=con.createStatement();  
	ResultSet rs=stmt.executeQuery("select * from scholar");  
	
	while(rs.next()) 
	{		ScholarModel s=new ScholarModel();
	...
	public String postUpdateScholar(ScholarModel sch) { //Update Scholar Details
